...
/**
 * Derives a short market label from the scraper's free-text location string, so the
 * dashboard can group/filter by market without a hardcoded country list - whatever
 * markets a fork's own `/scrape` queries actually cover show up on their own. Takes
 * the last comma-separated segment ("Berlin, Germany" -> "Germany"), which is where
 * every portal in this repo's search-queries.md convention puts the country.
 */
...
async function readTrackerRows(trackerPath) {
  ...
  for (const line of lines.slice(1)) {
    ...
    byKey.set(trackerKey(company, role), {
      ...
      source: f[idx.source] ?? "",
      // Optional column some forks add via a dashboard write-back convention of their
      // own; read it if present, but never require it - absent means simply blank here.
...
function buildJobs(seen, trackerByKey) {
  return Object.entries(seen).map(([key, entry]) => {
    ...
    return {
      ...
      // Some forks' /rank writes a location_verdict field instead of the plain
      // location key (avoids colliding with the scraper's own place field of the
      // same name); support both so this reads correctly either way.
...
export async function buildDashboardData() {
  ...
  const jobs = buildJobs(data.seen ?? {}, trackerByKey)
...
export async function updateTrackerStatus({ company, role, status, note, source, fit_rating }, trackerPath = TRACKER_PATH) {
  ...
  // whatever the most recent status-relevant date is. Only applies when the tracker
  // already has this optional column.
...
(function () {
  ...
  // Markets aren't a fixed enum - they're whatever country names show up in the scraped
  // location text, so this list grows/shrinks on its own as search coverage changes.
  function marketOptions() {
    return Array.from(new Set(RAW.jobs.map(j => j.market).filter(Boolean))).sort().map(m => ({ value: m, label: m }));
  }
  ...
  function renderTable(list) {
    ...
    for (const job of list) {
      ...
      tr.appendChild(el('td', { html: job.market ? badge(job.market, 'badge-gray') : '—' }));
  ...
  function render() {
    ...
    renderTable(sorted);
    updateSortArrows();
    renderMultiSelect('filter-market', 'Market', marketOptions(), state.market);
