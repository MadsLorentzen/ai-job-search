...
2. **Never fabricate candidate facts.** Every claim in a CV or cover letter must
   trace to `profile/profile.md`, `profile/workflows/apply.md` sources, or the
...
3. **State moves through tools, not context.** `state/seen_jobs.json`,
   `state/job_search_tracker.csv`, and `documents/applications/` are read and
...
4. **Language gate.** A posting's required language is checked against the
   ...
   (see `profile/workflows/` job-evaluation rules).
...
- **Scrape/rank/apply/outcome** each follow the spec in `profile/workflows/<name>.md`.
