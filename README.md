...
A structured workflow that turns Claude Code into a full-stack job application assistant. The core workflow (self-profiling, fit evaluation, and the drafter-reviewer application pipeline) is **language- and country-agnostic**. **This fork is configured for the US market**: LinkedIn, Indeed, Dice, Built In, Wellfound, ClearanceJobs, USAJobs, freehire.me, and employer ATS boards on Greenhouse, Lever, Ashby, and Workday. Danish portal CLIs still ship, disabled, so you can pull upstream updates without them becoming the default search path.
...
- Optional: `pdftotext` from [poppler](https://poppler.freedesktop.org/) (macOS: `brew install poppler`, Debian/Ubuntu: `apt install poppler-utils`, Windows: `choco install poppler`). `/apply` uses it for the ATS parseability check on the compiled CV. If missing, the check degrades gracefully to a visual keyword review.
...
For `linkedin-search` and `freehire-search` the install is optional: both have zero runtime dependencies and run with plain `bun`; `bun install` only pulls TypeScript dev types. `ats-autofill` needs Playwright Chromium. Danish portal CLIs are installed the same way if you later enable them.
...
This searches multiple job portals for positions matching your profile, deduplicates results, and presents them sorted by fit. Pick a match to run `/apply` on it directly. When a scrape returns more jobs than you want to eyeball, run `/rank` first to batch-score them all against the fit framework and get a ranked shortlist.
...
│   ├── linkedin-search/               # LinkedIn public job listings (enabled)
│   ├── freehire-search/               # freehire.me tech aggregator (enabled)
│   ├── ats-autofill/                  # US ATS form prefill (Greenhouse/Lever/Ashby/Workday)
│   ├── jobbank-search/                # Danish demo, disabled
│   ├── jobdanmark-search/             # Danish demo, disabled
│   ├── jobindex-search/               # Danish demo, disabled
│   └── jobnet-search/                 # Danish demo, disabled
├── application_profile.example.json   # Autofill profile template (copy, don't edit)
├── cv/
...
Discovery defaults to US boards. `/scrape` runs `linkedin-search` and `freehire-search`, then WebSearch against Indeed, Dice, Built In, Wellfound, ClearanceJobs, USAJobs, and employer ATS boards. The four Danish CLI tools ship **disabled**. To add another US board, run:
