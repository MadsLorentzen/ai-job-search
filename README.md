...
- **`/generate-csv`** chains `/scrape` and `/rank` and exports the day's results to `job_scraper/exports/jobs_YYYY-MM-DD.csv`, so a daily check is one command and one file instead of scrolling chat. Same defaults as `/scrape`/`/rank` (top-3 categories, new-only ranking); pass `broad` or a focus area to narrow or widen it.
...
│   │   ├── generate-csv.md            # /generate-csv scrape + rank + export today's jobs to CSV
