...
- **`/upskill`** analyzes the gap between your profile and your tracked job postings (or a single posting via `/upskill <URL>`). Produces a prioritized heatmap of skill gaps, a learning plan with web-searched study resources and time estimates, and a Quick Wins list of fast, certificate-backed gaps ranked by ROI (priority ÷ hours) that are worth closing right away — with exactly where to add each one (e.g. LinkedIn Certifications). Useful for career planning between applications.
