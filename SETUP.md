...
The job portal CLIs (`linkedin-search`, `freehire-search`, and optional Danish demos) plus `ats-autofill` are written in TypeScript and run with Bun. This fork's default market is the United States.
...
For `linkedin-search` and `freehire-search` the install is optional: both have zero runtime dependencies and run with plain `bun`; `bun install` only pulls TypeScript dev types. `ats-autofill` needs Playwright Chromium.

Danish portal CLIs remain in the tree but ship disabled. Enable them only if you want those boards. Add another US board with `/add-portal`.
