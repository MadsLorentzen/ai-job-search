...
1. **Dedupe.** Match tracker rows against ranked entries on case-insensitive company + role (casefold + strip on both fields) — the same match `/notion-sync`'s Step 2 describes and `tools/auto_mode_browser.py`'s `_tracker_keys` implements. A job present in both counts once.
