...
`deadline` is a **base field** rather than a `/rank` extension: Step 2's detail fetch already extracts the application deadline, so it is written when the job is first seen and refreshed by `/rank` Step 4 when a scoring agent returns a different value. The two absences mean different things and neither is a value: `null` means the posting states no deadline, a missing key means the entry predates this field. **Never infer a deadline** from either, and never backfill by guessing - a fabricated date drives the urgency marker and the expiry check, so a guess here silently expires a live posting or hides a closing one.
