...
If the user picks a number, invoke the **job-application-assistant** skill workflow (fit evaluation first, then CV + cover letter if approved). When an employer ATS link exists (Greenhouse, Lever, Ashby, Workday), offer `/autofill <ats_url>` after `/apply`.
