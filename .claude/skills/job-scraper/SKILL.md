...
`.claude/skills/job-application-assistant/09-web-research.md` before falling back to search snippet metadata. Never guess or invent requirements/deadlines.
...
Present new jobs in a table sorted by fit (high first), with **Remote roles presented first** within each fit tier. When Step 1b skipped
...
(bun unavailable, or its CLI failed at runtime, or portals without CLIs), report it with the
