...
- **Job portals:** "This fork searches the US market: LinkedIn, freehire.me, Indeed, Dice, Built In, Wellfound, ClearanceJobs, USAJobs, and employer ATS boards. Danish portal demos ship **disabled**. Which of those do you want, and do you use other US boards?" **Then act on the answer:** leave the four Danish `SKILL.md` files at `enabled: false` unless the user explicitly asks for Denmark. If they name a US board that isn't covered, offer `/add-portal` or add it to the site-specific queries in `search-queries.md`. WebSearch/`site:` queries remain the fallback for portals without a CLI skill.
