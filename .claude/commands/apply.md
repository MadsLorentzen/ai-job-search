...
- Extract: **company name**, **role title**, **department** (if mentioned), **location**, **work arrangement** (remote / hybrid / onsite), **application deadline** (if the posting states one), any **work-authorization or clearance requirements**, and **language** of the posting (usually English for US roles).
...
- **Match the language of the job posting** (English is the default for this US fork; a non-English posting gets a cover letter in that language)
...
### Next Steps
- **ATS form ready?** If an employer ATS link exists (Greenhouse, Lever, Ashby, Workday), offer `/autofill <ats_url>`. Those forms can be prefilled. LinkedIn, Indeed, and Dice must be applied to by hand.
