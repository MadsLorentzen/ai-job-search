...
4. **Status precedence:** the tracker wins. A job that is `ranked` in `seen_jobs.json` but `interview` in the tracker syncs as `interview`. Jobs only in `seen_jobs.json` keep their stored status.
   ...
   **Deadline precedence:** the tracker wins for the same reason, and for one more. Both sources can now carry a `deadline` and they can disagree, because each was read from the posting at a different time. The tracker's was captured by `/apply` at the moment the application was drafted, which is the deadline the user actually applied against. Use `seen_jobs.json`'s only when the tracker row has none, and omit the property when neither states one - never reconcile the two by picking the earlier or later date.
